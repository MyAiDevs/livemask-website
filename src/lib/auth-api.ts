/**
 * Auth API Client — TASK-WEBSITE-AUTH-HTTP-CLIENT-001
 *
 * Single responsibility: authentication bootstrap, token refresh, logout.
 * Protected endpoints (user profile, billing, devices) are accessed via
 * authClient which wraps authFetch() from the unified HTTP client.
 *
 * No business API file should call fetch() directly.
 * Only this file and http-client.ts may call raw fetch().
 */
...
const AUTH_ERRORS_MAP: Record<string, string> = {
  ...
  DEVICE_LIMIT_EXCEEDED:
    "Device limit reached. Please remove a device before adding a new one.",
  BILLING_PLAN_NOT_FOUND: "Selected plan was not found. Please try again.",
  BILLING_CHECKOUT_NOT_SUPPORTED:
    "Checkout is not yet supported. Payment integration coming soon.",
  AUTH_SESSION_EXPIRED: "Session expired. Please login again.",
};

function getErrorMessage(error: ApiError): string {
  return (
    AUTH_ERRORS_MAP[error.code] ||
    error.message ||
...
class AuthApiClient {
  ...
  private mockPlans: Plan[] = [
    ...
    },
    {
      ...
      device_limit: 5,
      node_access: "all",
      features: ["5 devices", "All nodes", "Priority speed"],
    },
    {
  ...
  private mockBillingHistory: BillingHistoryItem[] = [
    ...
    {
      invoice_id: "inv_mock_002",
      plan_id: "premium_monthly",
      amount_cents: 999,
      currency: "USD",
      status: "paid",
      paid_at: "2026-03-17T10:00:00Z",
      created_at: "2026-02-18T10:00:00Z",
    },
    {
      invoice_id: "inv_mock_003",
      plan_id: "premium_monthly",
      amount_cents: 999,
      currency: "USD",
      status: "paid",
      paid_at: "2026-02-17T10:00:00Z",
  ...
  private async mockRequest<T>(
    ...
    switch (path) {
      ...
      case "/api/v1/auth/login": {
        ...
        const isAdminLogin =
          req.client_type === "admin" && req.email === "admin@livemask.io";
      ...
      case "/api/v1/nodes/recommended": {
        const recommended = this.mockNodes.filter(
          (n) => n.status === "active" && !n.degraded,
        );
      ...
      case "/api/v1/devices": {
        if (options.method === "POST") {
          const newDevice: DeviceView = {
            ...
            device_name:
              (body as { device_name: string }).device_name || "New Device",
      ...
      default: {
        ...
        if (path.startsWith("/api/v1/devices/")) {
          ...
          this.mockDevices = this.mockDevices.filter(
        ...
        throw {
          status: 404,
          code: "NOT_FOUND",
          message: "Endpoint not found in mock",
        } as ApiError;
      }
    }
  }
  ...
  async getRecommendedNodes(): Promise<RecommendedNodeResponse> {
    return this.authRequest<RecommendedNodeResponse>(
      "/api/v1/nodes/recommended",
      {},
    );
  }
  ...
  async getSubscription(): Promise<SubscriptionView> {
    const res = await this.authRequest<{ subscription: SubscriptionView }>(
  ...
  async getBillingHistory(): Promise<{ items: BillingHistoryItem[] }> {
    return this.authRequest<{ items: BillingHistoryItem[] }>(
  ...
  async createCheckoutSession(
    ...
    return this.authRequest<{
      ...
      method: "POST",
      body: JSON.stringify({ plan_id: planId, payment_method: "mock" }),
    });
  }
  ...
  async addDevice(
    ...
    return this.authRequest<DeviceView>("/api/v1/devices", {
      method: "POST",
